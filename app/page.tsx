export default function Home() {
  const projects = [
    ...
    {
      title: "Real Estate Portfolio Dashboard",
      stack: "Power BI • DAX • CSV Data Modeling",
      bullets: [
      ...
      github: "",
      demo: "/real-estate-dashboard.pdf",
    },
    {
      ...
      <main className="mx-auto max-w-5xl px-6 py-20">
        <section className="mb-14">
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <a href="/resume.pdf" className="rounded-lg bg-black px-6 py-3 text-white hover:bg-zinc-800 dark:bg-white dark:text-black dark:hover:bg-zinc-200">
              Resume
            </a>
            <a href="https://github.com/sidbijo" target="_blank" rel="noopener noreferrer" className="rounded-lg border border-zinc-300 px-6 py-3 hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900">
              GitHub
            </a>
            <a href="https://www.linkedin.com/in/sidharthbijo/" target="_blank" rel="noopener noreferrer" className="rounded-lg border border-zinc-300 px-6 py-3 hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900">
              LinkedIn
            </a>
          </div>
        </section>
        ...
        <section>
          <div className="mb-6 flex items-end justify-between gap-4">
            ...
            <span className="text-sm text-zinc-500 dark:text-zinc-400">Selected work</span>
          </div>
          <div className="grid gap-6">
            {projects.map((p) => (
              <article key={p.title} className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  <div>
                    <h3 className="text-lg font-semibold">{p.title}</h3>
                    <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">{p.stack}</p>
                  </div>
                  <div className="flex gap-3">
                    {p.github ? (
                      <a href={p.github} target="_blank" rel="noopener noreferrer" className="rounded-md border border-zinc-300 px-3 py-2 text-sm hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800">
                        GitHub
                      </a>
                    ) : null}
                    {p.demo ? (
        ...
        <footer className="mt-16 text-sm text-zinc-500 dark:text-zinc-400">
          <p>© {new Date().getFullYear()} Sidharth Bijo • Built with Next.js & Vercel</p>
        </footer>
      </main>
    </div>
  );
